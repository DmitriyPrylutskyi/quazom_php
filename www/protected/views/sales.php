    <!-- At the Top of Page Area Starts -->
        <div class="top-page"><a href="#top" class="smoothscroll"><span><i class="fa fa-angle-up"></i></span></a></div>
    <!-- At the Top of Page Area Ends -->
    <!-- Hero Area Starts -->
    <div class="hero-area home-1">
        <div class="hero-area-content">
            <div class="speaker"><img src="/img/speaker.png" alt="Speaker"></div>
            <div class="headphone"><img src="/img/headphone.png" alt="Headphone"></div>
            <div class="pencil"><img src="/img/pencil.png" alt="Pencil"></div>
            <div class="frame"><img src="/img/frame.png" alt="Frame"></div>
            <div class="black-tab"><img src="/img/tab.png" alt="Tab"></div>
            <div class="mouse"><img src="/img/mouse.png" alt="Mouse"></div>
            <div class="hero-area-content-inner">
                <div class="container">
                    <div class="row">
                        <img src="/img/sales.png" alt="<?=$this->t('app', 'интернет магазин')?>">
                        <h1 class="text-center"><?=$this->t('app', 'Увеличьте свои продажи через интернет')?></h1>
                        <h3 class="text-center"><?=$this->t('app', 'Продающий Лендинг или автоматизированный Интернет Магазин помогут в этом!')?></h3>
						<div class="row">
							<form action="/call.php" method="post" id="contact-form-call" class="col-xs-12 col-md-8 col-md-offset-2" >
                                <input type="text" name="visitor_phone" placeholder="<?=$this->t('app', 'Номер телефона')?>" required class="form-control">
                                <button type="submit" class="btn btn-default btn-blue-fill-radius"><i class="fa fa-phone"></i><?=$this->t('app', 'Получите увеличение Ваших продаж')?></button>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Hero Area Ends -->
    <!-- Begin Area Starts -->
    <div class="begin-area">
        <div class="container">
            <div class="row">
                <div class="col-xs-12 col-sm-10 col-sm-offset-1">
                    <div class="section-title-1">
                        <h2 class="title wow fadeInDown"><?=$this->t('app', 'Как этого достичь?')?></h2>
                    </div>
                </div>
            </div>
            <div class="row block">
                <div class="col-xs-12 col-sm-2 img wow fadeInLeft" data-wow-delay="0.2s">
                    <img src="/img/svg/sales-reason-1.svg" alt="<?=$this->t('app', 'продажи через интернет')?>">
                </div>
                <div class="col-xs-12 col-sm-10 body wow fadeInRight" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'Мы подберем Вам оптимальную структуру интернет магазина или landing page, исходя из потребностей Вашей целевой аудитории. Ваши продажи в интернете смогут идти параллельно Вашему основному офлайн магазину. Также есть все возможности и для варианта дропшиппинга.')?></p>
                </div>
            </div>
            <div class="row block">
                <div class="col-xs-12 col-sm-10 body wow fadeInLeft" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'Мы поможем запустить релиз Вашего флагманского продукта или события с помощью яркой и продающей Промо страницы. Правильная маркетинговая стратегия для Вашего сегмента и грамотное ценообразование существенно увеличат Ваши продажи.')?></p>
                </div>
                <div class="col-xs-12 col-sm-2 text-right img wow fadeInRight" data-wow-delay="0.2s">
                    <img src="/img/svg/sales-reason-2.svg" alt="<?=$this->t('app', 'заказать лендинг')?>">
                </div>
            </div>
            <div class="row block">
                <div class="col-xs-12 col-sm-2 img wow fadeInLeft" data-wow-delay="0.2s">
                    <img src="/img/svg/sales-reason-3.svg" alt="<?=$this->t('app', 'интернет магазин разработка')?>">
                </div>
                <div class="col-xs-12 col-sm-10 body wow fadeInRight" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'Автоматизация процессов продаж и интеграция платежных систем нами позволит освободить много времени Вашей работы. Также мы гарантировано сопровождаем Ваш сайт техподдержкой первый месяц после старта. Организуем при небоходимости контекстную рекламу Вашего магазина.')?></p>
                </div>
            </div>
        </div>
    </div>
    <!-- Begin Plan Area Ends -->
    <!-- Blank Background Area Starts -->
    <div class="blank-bg-area second"></div>
    <!-- Blank Background Area Ends -->
    <!-- Algoritm Area Starts -->
    <div class="algoritm-area">
        <div class="container">
            <div class="row">
                <div class="col-xs-12 col-sm-10 col-sm-offset-1">
                    <div class="section-title-1">
                        <h2 class="title wow fadeInDown"><?=$this->t('app', 'Алгоритм нашей работы с Вами')?></h2>
                    </div>
                </div>
            </div>
            <div class="row block">
                <div class="col-sm-2 col-md-1 img wow fadeInLeft" data-wow-delay="0.2s">
                    <img src="/img/svg/number-1.svg" alt="<?=$this->t('app', 'бриф сайта')?>">
                </div>
                <div class="col-sm-10 col-md-11 body wow fadeInRight" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'Вы скачиваете и заполняете Анкету для создания Вашего промо сайта или интернет магазина, затем отправляете нам.')?></p>
                </div>
            </div>
            <div class="row block">
                <div class="col-sm-2 col-sm-offset-1 col-md-1 img wow fadeInLeft" data-wow-delay="0.2s">
                    <img src="/img/svg/number-2.svg" alt="<?=$this->t('app', 'примеры сайта')?>">
                </div>
                <div class="col-sm-9 col-md-10 body wow fadeInRight" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'ы вместе обсуждаем Анкету, задачи и структуру Вашего сайта, смотрим примеры, заключаем договор и оформляем предоплату.')?>М</p>
                </div>
            </div>
            <div class="row block">
                <div class="col-sm-2 col-sm-offset-2 col-md-1 img wow fadeInLeft" data-wow-delay="0.2s">
                    <img src="/img/svg/number-3.svg" alt="<?=$this->t('app', 'прототип сайта')?>">
                </div>
                <div class="col-sm-8 col-md-9 body wow fadeInRight" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'Дальше мы создаем прототип со структурой Вашего будущего сайта. Вы вносите предложения по корректировке прототипа.')?></p>
                </div>
            </div>
            <div class="row block">
                <div class="col-sm-8 col-md-9 body wow fadeInLeft" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'После редактирования прототипа Вы принимаете данный вариант. Мы начинаем разрабатывать и настраивать Ваш продающий сайт.')?></p>
                </div>
                <div class="col-sm-2 col-md-1 img wow fadeInRight" data-wow-delay="0.2s">
                    <img src="/img/svg/number-4.svg" alt="<?=$this->t('app', 'настройка сайта')?>">
                </div>
            </div>
            <div class="row block">
                <div class="col-sm-9 col-md-10 body wow fadeInLeft" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'Вы принимаете готовый сайт, проверяете его работоспособность. Мы вместе обсуждаем и подбираем свободное имя домена.')?></p>
                </div>
                <div class="col-sm-2 col-md-1 img wow fadeInRight" data-wow-delay="0.2s">
                    <img src="/img/svg/number-5.svg" alt="<?=$this->t('app', 'создание сайта')?>">
                </div>
            </div>
            <div class="row block">
                <div class="col-sm-10 col-md-11 body wow fadeInLeft" data-wow-delay="0.2s">
                    <p><?=$this->t('app', 'После полной оплаты нашей работы мы выкладываем Ваш сайт на выбранный Вами домен и хостинг. Вы увеличиваете свои продажи в интернете.')?></p>
                </div>
                <div class="col-sm-2 col-md-1 img wow fadeInRight" data-wow-delay="0.2s">
                    <img src="/img/svg/number-6.svg" alt="<?=$this->t('app', 'размещение сайта')?>">
                </div>
            </div>
        </div>
    </div>
    <!-- Algoritm Plan Area Ends -->
    <!-- App Download Area -->
    <div class="app-download-area">
        <div class="container">
            <div class="row">
                <div class="col-xs-12 col-sm-10 col-sm-offset-1">
                    <div class="app-download-content-2">
                        <h2 class="col-xs-12 col-sm-10 title wow fadeInleft" data-wow-delay="0.2s"><?=$this->t('app', 'Получите Анкету для создания сайта')?></h2>
                        <div class="col-xs-12 col-sm-2 icon wow fadeInRight" data-wow-delay="0.7s">
                            <form action="/download.php" method="post" id="download">
                                <button type="submit" name="<?=$this->getLang()?>" class="btn btn-default btn-blue-fill-radius"><?=$this->t('app', 'Скачать')?></button>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- App Download Area Ends -->